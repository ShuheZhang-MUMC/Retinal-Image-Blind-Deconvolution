...
% mask indicates region with gradients. outside of mask should be smooth...
...
H = size(denoised,1);    W = size(denoised,2);
otf_dx = psf2otf(dx,size(denoised));
otf_dy = psf2otf(dy,size(denoised));
Bx_temp = real(ifft2(fft2(denoised).*otf_dx));px = Bx_temp(1:H-1,1:W-1);
By_temp = real(ifft2(fft2(denoised).*otf_dy));py = By_temp(1:H-1,1:W-1);
